...
function stagekitwp_sync_page_details( $embedded = false ) {
    stagekitwp_sync_cap_check();

    $close_wrapper = function () use ( $embedded ) {
        if ( ! $embedded ) {
            echo '</div>';
        }
    };
    ...
    if ( ! $embedded ) {
        echo '<div class="wrap"><h1>Sync · Details</h1>';
        echo stagekitwp_sync_render_hub_tabs( 'details' );
    }
    ...
    $secret    = get_option(STAGEKITWP_SYNC_CLIENT_SECRET_OPT, '');
    ...
    if ( ! $access_token ) {
        ...
        stagekitwp_log_error('Details page: failed to retrieve access token', ['response' => $token_data], ['channel' => 'auth']);
        $close_wrapper();
        return;
    }
    ...
    if ( isset($lists_data['value']) && is_array($lists_data['value']) ) {
        ...
        stagekitwp_log_error('Details page: failed to retrieve SharePoint lists', ['response' => $lists_data], ['channel' => 'auth']);
        $close_wrapper();
        return;
    }
    ...
    stagekitwp_log_info('Details page: checked SharePoint list existence', [
