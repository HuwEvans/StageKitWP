...
class STAGEKITWP_Graph_Client {
    ...
    public function __construct() {
        ...
        $opts = get_option('stagekitwp_sync_auth', []);
        ...
        error_log('[STAGEKITWP_Graph_Client] Constructor: SiteID=' . ($this->site_id ? $this->site_id : 'EMPTY') .
    ...
    private function request($endpoint) {
        ...
        if (!$token) {
            error_log('[STAGEKITWP_Graph_Client] Cannot make request - no access token available');
            return null;
        }

        $url = "https://graph.microsoft.com/v1.0/$endpoint";
        error_log('[STAGEKITWP_Graph_Client] Making request to: ' . $endpoint);
        error_log('[STAGEKITWP_Graph_Client] Full URL: ' . $url);
        ...
        if (is_wp_error($response)) {
            error_log('[STAGEKITWP_Graph_Client] Request error: ' . $response->get_error_message());
            return null;
        }
        ...
        if ($code !== 200) {
            error_log('[STAGEKITWP_Graph_Client] Request failed (HTTP ' . $code . '): ' . $body);
            return null;
        }

        $data = json_decode($body, true);
        error_log('[STAGEKITWP_Graph_Client] Request succeeded, response type: ' . gettype($data));
        error_log('[STAGEKITWP_Graph_Client] Response value count: ' . (isset($data['value']) ? count($data['value']) : 'N/A'));
        return $data;
    }
    ...
    private function get_access_token() {
        ...
        error_log('[STAGEKITWP_Graph_Client] Attempting to obtain new access token');

        // Get credentials
        $opts = get_option('stagekitwp_sync_auth', []);
        $tenant = $opts['tenant_id'] ?? null;
        $client_id = $opts['client_id'] ?? null;
        $secret = get_option('stagekitwp_sync_client_secret');

        if (!$tenant || !$client_id || !$secret) {
            error_log('[STAGEKITWP_Graph_Client] Missing credentials: tenant=' . ($tenant ? 'SET' : 'EMPTY') .
        ...
        if (is_wp_error($response)) {
            error_log('[STAGEKITWP_Graph_Client] Token request error: ' . $response->get_error_message());
            return null;
        }
        ...
        if ($code !== 200 || empty($body['access_token'])) {
            error_log('[STAGEKITWP_Graph_Client] Token request failed (HTTP ' . $code . '): ' . 
                      ($body['error_description'] ?? $body['error'] ?? 'Unknown error'));
            return null;
        }

        $this->access_token = $body['access_token'];
        error_log('[STAGEKITWP_Graph_Client] Token obtained successfully, expires in ' . ($body['expires_in'] ?? 'unknown') . ' seconds');

        return $this->access_token;
    }

    public function get_list_items($list_name) {
        error_log('[STAGEKITWP_Graph_Client] get_list_items called for: ' . $list_name);

        if (!$this->site_id) {
            error_log('[STAGEKITWP_Graph_Client] Missing Site ID');
            return null;
        }
        ...
        if (!$list_id) {
            error_log('[STAGEKITWP_Graph_Client] Failed to resolve list ID for: ' . $list_name);
            return null;
        }

        error_log('[STAGEKITWP_Graph_Client] Resolved list ID: ' . $list_id);
        ...
        error_log('[STAGEKITWP_Graph_Client] Using endpoint with fields: ' . $list_name);
        error_log('[STAGEKITWP_Graph_Client] Requesting fields: ' . implode(', ', $fields));
        error_log('[STAGEKITWP_Graph_Client] Full endpoint: ' . $endpoint);
        
        $data = $this->request($endpoint);
        
        if (!$data) {
            error_log('[STAGEKITWP_Graph_Client] No data returned from endpoint with field selection, trying fallback without field selection');
            
            // Fallback: try without explicit field selection
            $fallback_endpoint = "sites/{$this->site_id}/lists/{$list_id}/items?expand=fields";
            error_log('[STAGEKITWP_Graph_Client] Trying fallback endpoint: ' . $fallback_endpoint);
            $data = $this->request($fallback_endpoint);
            
            if ($data) {
                error_log('[STAGEKITWP_Graph_Client] Fallback succeeded - returned data');
            } else {
                error_log('[STAGEKITWP_Graph_Client] Fallback also failed - no data returned');
                return null;
            }
        }
        
        if (!$data) {
            error_log('[STAGEKITWP_Graph_Client] No data returned from endpoint');
            return null;
        }
        
        error_log('[STAGEKITWP_Graph_Client] Successfully fetched items, count: ' . count($data['value'] ?? []));
        return $data['value'] ?? [];
    }
    ...
    private function resolve_list_id($list_name) {
        error_log('[STAGEKITWP_Graph_Client] Resolving list ID for: ' . $list_name);
        
        $endpoint = "sites/{$this->site_id}/lists";
        $lists = $this->request($endpoint);
        
        if (!$lists || !isset($lists['value'])) {
            error_log('[STAGEKITWP_Graph_Client] Failed to fetch lists from SharePoint');
            return null;
        }
        
        error_log('[STAGEKITWP_Graph_Client] Found ' . count($lists['value']) . ' lists in SharePoint');
        ...
        error_log('[STAGEKITWP_Graph_Client] Available lists: ' . implode(', ', $available_lists));
        
        foreach ($lists['value'] ?? [] as $list) {
            if ($list['name'] === $list_name) {
                error_log('[STAGEKITWP_Graph_Client] Found matching list: ' . $list['id']);
                return $list['id'];
            }
        }
        
        error_log('[STAGEKITWP_Graph_Client] List name not found in SharePoint: ' . $list_name);
        // Log a suggestion of what was found
        error_log('[STAGEKITWP_Graph_Client] Searched for: "' . $list_name . '", but lists are: ' . implode(', ', $available_lists));
        return null;
    }
    ...
    public function download_sharepoint_file($file_url) {
        if (empty($file_url)) {
            error_log('[STAGEKITWP_Graph_Client] download_sharepoint_file: Empty URL provided');
            return null;
        }

        $token = $this->get_access_token();
        if (!$token) {
            error_log('[STAGEKITWP_Graph_Client] download_sharepoint_file: Failed to get access token');
            return null;
        }
        ...
        if (empty($path)) {
            error_log('[STAGEKITWP_Graph_Client] download_sharepoint_file: Could not extract path from URL: ' . $file_url);
            return null;
        }
        ...
        error_log('[STAGEKITWP_Graph_Client] Downloading file via Graph API. Path: ' . $path . ' URL: ' . $graph_url);
        ...
        if (is_wp_error($response)) {
            error_log('[STAGEKITWP_Graph_Client] download_sharepoint_file error: ' . $response->get_error_message());
            return null;
        }
        ...
        if ($http_code !== 200) {
            $body = wp_remote_retrieve_body($response);
            error_log('[STAGEKITWP_Graph_Client] download_sharepoint_file HTTP ' . $http_code . ': ' . substr($body, 0, 300));
            return null;
        }

        $file_content = wp_remote_retrieve_body($response);
        if (empty($file_content)) {
            error_log('[STAGEKITWP_Graph_Client] download_sharepoint_file: Empty response body');
            return null;
        }

        error_log('[STAGEKITWP_Graph_Client] File downloaded successfully, size: ' . strlen($file_content) . ' bytes');
        return $file_content;
    }
    ...
    private function get_fields_for_list($list_name) {
        ...
        error_log('[STAGEKITWP_Graph_Client] Field map for ' . $list_name . ': ' . json_encode($fields));
        return $fields;
    }
}
