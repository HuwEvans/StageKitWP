...
stagekitwp_sync_log('INFO','Folder-Discovery File loading');
 ...
 */
function stagekitwp_sync_extract_folder_from_url($image_url) {
    if (empty($image_url)) {
        return null;
    }
    ...
    if (isset($matches[1])) {
        ...
        stagekitwp_sync_log('debug', 'Extracted folder from URL', [
            'url' => $image_url,
            'folder' => $folder_name
        ]);
        return $folder_name;
    ...
    stagekitwp_sync_log('warning', 'Could not extract folder from URL', [
        'url' => $image_url,
        'path' => $path
    ]);
    return null;
 ...
 */
function stagekitwp_sync_get_cached_folder_ids() {
    $cached = get_option('stagekitwp_sync_image_media_folder_ids', []);
 ...
 */
function stagekitwp_sync_cache_folder_id($folder_name, $folder_id) {
    $cached = stagekitwp_sync_get_cached_folder_ids();
    $cached[$folder_name] = $folder_id;
    update_option('stagekitwp_sync_image_media_folder_ids', $cached);
    
    stagekitwp_sync_log('debug', 'Cached folder ID', [
 ...
 */
function stagekitwp_sync_clear_folder_cache() {
    delete_option('stagekitwp_sync_image_media_folder_ids');
    stagekitwp_sync_log('info', 'Cleared folder ID cache');
}
 ...
 */
function stagekitwp_sync_get_folder_id($folder_name, $site_id, $image_media_list_id, $token) {
    if (empty($folder_name)) {
        stagekitwp_sync_log('warning', 'Folder name is empty');
        return null;
    }

    // TIER 1: Check manual overrides first (highest priority)
    $overrides = get_option('stagekitwp_sync_folder_overrides', []);
    if (isset($overrides[$folder_name])) {
        stagekitwp_sync_log('debug', 'Using manual override for folder', [
            'folder_name' => $folder_name,
    ...
    // TIER 2: Check cache
    $cached_ids = stagekitwp_sync_get_cached_folder_ids();
    if (isset($cached_ids[$folder_name])) {
        stagekitwp_sync_log('debug', 'Using cached folder ID', [
            'folder_name' => $folder_name,
    ...
    stagekitwp_sync_log('debug', 'Folder not in cache/overrides, discovering from SharePoint', [
        'folder_name' => $folder_name
    ]);

    $folder_id = stagekitwp_sync_discover_folder_id($folder_name, $site_id, $image_media_list_id, $token);
    
    if ($folder_id) {
        // Cache for future use
        stagekitwp_sync_cache_folder_id($folder_name, $folder_id);
        stagekitwp_sync_log('info', 'Discovered and cached folder ID', [
            'folder_name' => $folder_name,
            'folder_id' => $folder_id,
    ...
    stagekitwp_sync_log('warning', 'Folder not found in SharePoint or overrides', [
        'folder_name' => $folder_name
    ]);
    return null;
}
 ...
 * @return string|null Folder ID if found, null otherwise
 */
function stagekitwp_sync_discover_folder_id($folder_name, $site_id, $image_media_list_id, $token) {
    if (empty($folder_name)) {
        return null;
    }

    stagekitwp_sync_log('debug', 'Searching for folder in Image Media', [
        'folder_name' => $folder_name
    ]);
    ...
    if (is_wp_error($response)) {
        stagekitwp_sync_log('error', 'Failed to list Image Media root folder', [
    ...
    if (!isset($json['value'])) {
        stagekitwp_sync_log('warning', 'No items in Image Media root', [
            'folder_name' => $folder_name
        ]);
        return null;
    }
    ...
    foreach ($json['value'] as $item) {
        if (strtolower($item['name']) === strtolower($folder_name)) {
            stagekitwp_sync_log('info', 'Found folder in Image Media', [
                'folder_name' => $folder_name,
                'folder_id' => $item['id']
            ]);
            return $item['id'];
    ...
    stagekitwp_sync_log('warning', 'Folder not found in Image Media root', [
        'folder_name' => $folder_name,
 ...
 */
function stagekitwp_sync_discover_all_folders($site_id, $image_media_list_id, $token) {
    stagekitwp_sync_log('info', 'Discovering all folders in Image Media');
    ...
    if (is_wp_error($response)) {
        stagekitwp_sync_log('error', 'Failed to discover folders', [
            'error' => $response->get_error_message()
        ]);
        return [];
    }
    ...
    if (!empty($discovered)) {
        update_option('stagekitwp_sync_image_media_folder_ids', array_merge(
            stagekitwp_sync_get_cached_folder_ids(),
            $discovered
        ));
        
        stagekitwp_sync_log('info', 'Discovered and cached folders', [
 ...
 * @return string|null Folder ID if found, null otherwise
 */
function stagekitwp_sync_get_folder_id_from_image_url($image_url, $site_id, $image_media_list_id, $token) {
    // Extract folder name from URL
    $folder_name = stagekitwp_sync_extract_folder_from_url($image_url);
    
    if (!$folder_name) {
        stagekitwp_sync_log('warning', 'Could not extract folder name from image URL', [
            'url' => $image_url
        ]);
        return null;
    }
    ...
    return stagekitwp_sync_get_folder_id($folder_name, $site_id, $image_media_list_id, $token);
}
