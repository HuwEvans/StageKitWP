<?php
/**
 * StageKitWP - Season Shows Beaver Builder Module Frontend
 * 
 * Renders the season shows shortcode with module settings
 * 
 * @package StageKitWP
...
$hide_author = isset( $settings->hide_author ) && $settings->hide_author === 'true' ? 'true' : 'false';
$hide_director = isset( $settings->hide_director ) && $settings->hide_director === 'true' ? 'true' : 'false';
$hide_producer = isset( $settings->hide_producer ) && $settings->hide_producer === 'true' ? 'true' : 'false';
$hide_stage_manager = isset( $settings->hide_stage_manager ) && $settings->hide_stage_manager === 'true' ? 'true' : 'false';
...
if ( ! empty( $orderby ) && 'title' !== $orderby ) {
...
if ( 'true' === $hide_director ) {
    $shortcode_atts[] = 'hide_director="true"';
}

if ( 'true' === $hide_author ) {
    $shortcode_atts[] = 'hide_author="true"';
}

if ( 'true' === $hide_producer ) {
    $shortcode_atts[] = 'hide_producer="true"';
}

if ( 'true' === $hide_stage_manager ) {
    $shortcode_atts[] = 'hide_stage_manager="true"';
}
...
$shortcode_string = '[stagekitwp_season_shows ' . implode( ' ', $shortcode_atts ) . ']';
