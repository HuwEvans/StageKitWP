 ...
 */
function stagekitwp_register_venues_block() {
    register_block_type('stagekitwp/venues', array(
        'render_callback' => 'stagekitwp_render_venues_block',
        'attributes' => array(
            'showMaps' => array(
                'type' => 'boolean',
                'default' => false,
...
add_action('init', 'stagekitwp_register_venues_block');

/**
 * Render the Venues block
 */
function stagekitwp_render_venues_block($attributes) {
    // Sanitize attributes
    ...
    wp_enqueue_style('stagekitwp-venues-block-css', plugins_url('venues-block.css', __FILE__), array(), '4.0.0');
    wp_enqueue_script('stagekitwp-venues-block-js', plugins_url('venues-block.js', __FILE__), array(), '4.0.0', true);
    ...
    if (!empty($show_id)) {
        $venue_id = get_post_meta(intval($show_id), '_stagekitwp_show_venue', true);
        if ($venue_id) {
            $venue_args['post__in'] = array($venue_id);
        } else {
            return '<div class="stagekitwp-venues-block stagekitwp-venues-empty"><p>No venue assigned to this show.</p></div>';
        }
    }
    ...
    if (empty($venues)) {
        return '<div class="stagekitwp-venues-block stagekitwp-venues-empty"><p>No venues found.</p></div>';
    }

    // Start output
    $output = '<div class="stagekitwp-venues-block" data-show-maps="' . esc_attr($show_maps ? '1' : '0') . '" data-columns="' . esc_attr($columns) . '">';

    // Search box (optional)
    if ($show_search && count($venues) > 1) {
        $output .= '<div class="stagekitwp-venues-search">';
        $output .= '<input type="text" class="stagekitwp-venues-search-input" placeholder="Search venues..." />';
        $output .= '</div>';
    }

    // Venues accordion
    $output .= '<div class="stagekitwp-venues-accordion">';

    $map_counter = 0;
    foreach ($venues as $venue) {
        $venue_id = $venue->ID;
        $venue_name = get_post_meta($venue_id, '_stagekitwp_venue_name', true);
        ...
        $venue_latitude = get_post_meta($venue_id, '_stagekitwp_venue_latitude', true);
        $venue_longitude = get_post_meta($venue_id, '_stagekitwp_venue_longitude', true);
        ...
        $output .= '<div class="stagekitwp-venue-accordion-item" data-venue-id="' . esc_attr($venue_id) . '" data-venue-name="' . esc_attr($venue_name) . '">';
        $output .= '<button class="stagekitwp-venue-accordion-header" aria-expanded="false" aria-controls="stagekitwp-venue-content-' . esc_attr($venue_id) . '">';
        $output .= '<span class="stagekitwp-venue-name">' . esc_html($venue_name) . '</span>';
        $output .= '<span class="stagekitwp-venue-toggle-icon">+</span>';
        $output .= '</button>';

        // Accordion item content
        $output .= '<div id="stagekitwp-venue-content-' . esc_attr($venue_id) . '" class="stagekitwp-venue-accordion-content" hidden>';
        ...
        if (!empty($venue_image)) {
            ...
            if ($image_url) {
                $output .= '<div class="stagekitwp-venue-image">';
        ...
        // Venue details
        $output .= '<div class="stagekitwp-venue-details">';

        if (!empty($venue_address)) {
            $output .= '<div class="stagekitwp-venue-detail-item stagekitwp-venue-address">';
        ...
        if (!empty($venue_phone)) {
            $output .= '<div class="stagekitwp-venue-detail-item stagekitwp-venue-phone">';
        ...
        if (!empty($venue_website)) {
            $output .= '<div class="stagekitwp-venue-detail-item stagekitwp-venue-website">';
        ...
        if (!empty($google_maps_url)) {
            $output .= '<div class="stagekitwp-venue-detail-item stagekitwp-venue-directions">';
            $output .= '<a href="' . esc_url($google_maps_url) . '" target="_blank" rel="noopener noreferrer" class="stagekitwp-btn stagekitwp-btn-directions">Get Directions</a>';
            $output .= '</div>';
        }
        ...
        if ($show_maps && !empty($venue_latitude) && !empty($venue_longitude)) {
            $output .= '<div class="stagekitwp-venue-map-container">';
            $output .= '<div id="' . esc_attr($map_id) . '" class="stagekitwp-venue-map"></div>';
            $output .= '</div>';
