...
stagekitwp_sync_log('INFO','Seasons-Sync File loading');

function stagekitwp_sync_fetch_seasons_data() {
    stagekitwp_sync_log('debug', 'Fetching SharePoint seasons data.');
    ...
    stagekitwp_sync_log('debug', 'Calling get_list_items for Seasons');
    ...
    if (empty($items)) {
        stagekitwp_sync_log('warning', 'SharePoint Seasons list is empty');
        return [];
    }

    stagekitwp_sync_log('info', 'Fetched SharePoint seasons data successfully.', ['item_count' => count($items)]);
    return $items;
}

function stagekitwp_sync_process_season($item, $dry_run = false) {
    try {
        stagekitwp_sync_log('debug', 'Processing season item.', ['item_id' => $item['id'] ?? 'unknown']);
        ...
        stagekitwp_sync_log('debug', 'Extracted fields', ['sp_id' => $sp_id, 'name' => $name, 'start_date' => $start_date, 'is_current' => $is_current_season, 'is_upcoming' => $is_upcoming_season]);
        ...
        stagekitwp_sync_log('debug', 'Processing season', ['name' => $name, 'sp_id' => $sp_id]);
        ...
        if ($dry_run) {
            stagekitwp_sync_log('debug', 'Dry-run: Would process season.', ['name' => $name, 'sp_id' => $sp_id]);
            return true;
        }
        ...
        if ($existing) {
            ...
            stagekitwp_sync_log('info', 'Updated season.', ['name' => $name, 'sp_id' => $sp_id, 'post_id' => $post_id]);
        } else {
            $post_id = wp_insert_post($post_data);
            if (is_wp_error($post_id)) {
                stagekitwp_sync_log('error', 'Failed to insert season post.', ['name' => $name, 'error' => $post_id->get_error_message()]);
                return false;
            }
            update_post_meta($post_id, '_stagekitwp_sp_id', $sp_id);
            stagekitwp_sync_log('info', 'Created season.', ['name' => $name, 'sp_id' => $sp_id, 'post_id' => $post_id]);
        }

        // Update season metadata
        update_post_meta($post_id, '_stagekitwp_season_name', $name);
        update_post_meta($post_id, '_stagekitwp_season_start_date', $start_date);
        update_post_meta($post_id, '_stagekitwp_season_end_date', $end_date);
        update_post_meta($post_id, '_stagekitwp_season_is_current', $is_current_season);
        update_post_meta($post_id, '_stagekitwp_season_is_upcoming', $is_upcoming_season);
        ...
        stagekitwp_sync_log('debug', 'Processing season images', [
            'post_id' => $post_id,
        ...
        if (!class_exists('STAGEKITWP_Graph_Client')) {
            stagekitwp_sync_log('warning', 'STAGEKITWP_Graph_Client not available for image sync');
        } else {
            $client = new STAGEKITWP_Graph_Client();
            ...
            if ($token) {
                ...
                if ($image_front_url) {
                    $attachment_id = stagekitwp_sync_download_and_attach_image(
                        $post_id,
                        $image_front_url,
                        '_stagekitwp_season_image_front',
                    ...
                    if ($attachment_id) {
                        ...
                        update_post_meta($post_id, '_stagekitwp_season_image_front_url', $image_front_url);
                ...
                if ($image_back_url) {
                    ...
                    if ($attachment_id) {
                        ...
                        // Only store URL as fallback if download failed
                        update_post_meta($post_id, '_stagekitwp_season_image_back_url', $image_back_url);
                ...
                if ($website_banner_url) {
                    $attachment_id = stagekitwp_sync_download_and_attach_image(
                        $post_id,
                        $website_banner_url,
                        '_stagekitwp_season_social_banner',
                    ...
                    if ($attachment_id) {
                        ...
                        // Only store URL as fallback if download failed
                        update_post_meta($post_id, '_stagekitwp_season_social_banner_url', $website_banner_url);
                ...
                if ($sm_square_url) {
                    ...
                    if ($attachment_id) {
                        ...
                        // Only store URL as fallback if download failed
                        update_post_meta($post_id, '_stagekitwp_season_sm_square_url', $sm_square_url);
                ...
                if ($sm_portrait_url) {
                    $attachment_id = stagekitwp_sync_download_and_attach_image(
                        $post_id,
                        $sm_portrait_url,
                        '_stagekitwp_season_sm_portrait',
                    ...
                    if ($attachment_id) {
                        ...
                        // Only store URL as fallback if download failed
                        update_post_meta($post_id, '_stagekitwp_season_sm_portrait_url', $sm_portrait_url);
        ...
        stagekitwp_sync_log('debug', 'Finished processing season.', ['sp_id' => $sp_id, 'post_id' => $post_id]);
        return true;
    } catch (Exception $e) {
        stagekitwp_sync_log('error', 'Exception processing season item: ' . $e->getMessage(), ['item_id' => $item['id'] ?? 'unknown', 'trace' => $e->getTraceAsString()]);
        return false;
    } catch (Throwable $t) {
        stagekitwp_sync_log('error', 'Error processing season item: ' . $t->getMessage(), ['item_id' => $item['id'] ?? 'unknown', 'trace' => $t->getTraceAsString()]);
        return false;
    }
}

function stagekitwp_sync_seasons($dry_run = false) {
    stagekitwp_sync_log('info', 'Starting seasons sync.', ['dry_run' => $dry_run]);

    $items = stagekitwp_sync_fetch_seasons_data();
    if (!$items) {
    ...
    foreach ($items as $item) {
        if (stagekitwp_sync_process_season($item, $dry_run)) {
            $count++;
        } else {
            stagekitwp_sync_log('warning', 'Failed to process season item.', ['item' => $item]);
        }
    }
    ...
    stagekitwp_sync_log('info', 'Seasons sync completed.', ['dry_run' => $dry_run, 'synced_count' => $count]);
    return $summary;
}
