<?php
/**
 * Image Management for StageKitWP Sync
 ...
 */
function stagekitwp_sync_get_images_dir() {
    $upload_dir = wp_upload_dir();
    return trailingslashit($upload_dir['basedir']) . 'stagekitwp-sync-images';
}
 ...
 */
function stagekitwp_sync_get_images_url() {
    $upload_dir = wp_upload_dir();
    return trailingslashit($upload_dir['baseurl']) . 'stagekitwp-sync-images';
}
 ...
 */
function stagekitwp_sync_create_images_folder() {
    $images_dir = stagekitwp_sync_get_images_dir();
    
    if (file_exists($images_dir)) {
        if (is_dir($images_dir) && is_writable($images_dir)) {
            stagekitwp_sync_log('debug', 'Synced images folder already exists and is writable', ['path' => $images_dir]);
            return true;
        }
    }
    ...
    if (!is_dir($images_dir)) {
        if (!wp_mkdir_p($images_dir)) {
            stagekitwp_sync_log('error', 'Failed to create synced images folder', ['path' => $images_dir]);
            return false;
        }
    }
    ...
    if (!file_exists($htaccess_file)) {
        ...
        if (@file_put_contents($htaccess_file, $htaccess_content) === false) {
            stagekitwp_sync_log('warning', 'Failed to create .htaccess in synced images folder', ['path' => $htaccess_file]);
    ...
    if (!file_exists($index_file)) {
        if (@file_put_contents($index_file, "<?php // Silence is golden\n") === false) {
            stagekitwp_sync_log('warning', 'Failed to create index.php in synced images folder', ['path' => $index_file]);
            // Don't fail - folder still works without it
        }
    }
    
    stagekitwp_sync_log('info', 'Successfully created synced images folder', ['path' => $images_dir]);
    return true;
}
 ...
 */
function stagekitwp_sync_delete_all_images() {
    $images_dir = stagekitwp_sync_get_images_dir();
    
    if (!file_exists($images_dir)) {
        stagekitwp_sync_log('info', 'Synced images folder does not exist - nothing to delete', ['path' => $images_dir]);
        return true;
    }
    ...
    if ($files) {
        foreach ($files as $file) {
            ...
            if (is_file($file)) {
                ...
                if (!stagekitwp_sync_delete_directory_recursive($file)) {
                    $errors[] = $file;
                }
            }
        }
    ...
    if (empty($errors)) {
        if (@rmdir($images_dir)) {
            stagekitwp_sync_log('info', 'Successfully deleted synced images folder and all contents', [
                'path' => $images_dir,
                'files_deleted' => $deleted_count
            ]);
            return true;
        } else {
            stagekitwp_sync_log('warning', 'Failed to delete synced images folder (files deleted but folder remains)', [
                'path' => $images_dir,
                'files_deleted' => $deleted_count
            ]);
            return false;
        ...
        stagekitwp_sync_log('error', 'Failed to delete some synced images', [
            'path' => $images_dir,
            'files_deleted' => $deleted_count,
            'failed_files' => $errors
        ]);
 ...
 */
function stagekitwp_sync_delete_directory_recursive($dir) {
    if (!is_dir($dir)) {
        return false;
    }
    ...
    if ($files) {
        foreach ($files as $file) {
            if (is_dir($file)) {
                stagekitwp_sync_delete_directory_recursive($file);
            } else {
                @unlink($file);
            }
        }
...
/**
 * Clear all StageKitWP Sync data when plugin is deactivated
 * Removes: synced images folder, scheduled events, etc.
 * 
 * @return void
 */
function stagekitwp_sync_deactivate_cleanup() {
    // Delete synced images
    stagekitwp_sync_delete_all_images();
    
    // Clear any scheduled hooks
    wp_clear_scheduled_hook('stagekitwp_sync_daily_check');
    
    stagekitwp_sync_log('info', 'StageKitWP Sync deactivated - cleanup completed');
}
 ...
 * @return void
 */
function stagekitwp_sync_activate_setup() {
    ...
    stagekitwp_sync_log('info', 'StageKitWP Sync activated - setup completed');
}
 ...
 */
function stagekitwp_sync_download_and_attach_image($post_id, $image_url, $meta_key, $prefix, $image_type = 'image') {
    if (empty($image_url) || empty($post_id)) {
        return null;
    }
    
    stagekitwp_sync_log('debug', "Downloading season image: $image_type", [
        'post_id' => $post_id,
    ...
    if ($existing_id && !empty($existing_id) && is_numeric($existing_id)) {
        ...
        if (get_post_type($attachment_id) === 'attachment') {
            stagekitwp_sync_log('debug', "Reusing existing $image_type attachment", [
                'post_id' => $post_id,
    ...
    stagekitwp_sync_log('debug', "Using download URL for season $image_type", [
        'post_id' => $post_id,
    ...
    if (is_wp_error($response)) {
        stagekitwp_sync_log('error', "Failed to download season $image_type", [
            'post_id' => $post_id,
    ...
    if ($http_code !== 200) {
        stagekitwp_sync_log('error', "Failed to download season $image_type - HTTP error", [
            'post_id' => $post_id,
            'http_code' => $http_code
        ]);
        return null;
    ...
    if (empty($file_content)) {
        stagekitwp_sync_log('error', "Downloaded season $image_type is empty", [
            'post_id' => $post_id
        ]);
        return null;
    }
    ...
    if (strpos($file_content, '<!DOCTYPE') === 0 || strpos($file_content, '<html') === 0) {
        stagekitwp_sync_log('error', "Downloaded season $image_type is HTML, not an image - possible permission issue", [
            'post_id' => $post_id,
    ...
    if (!file_put_contents($dest_file, $file_content)) {
        stagekitwp_sync_log('error', "Failed to write season $image_type to disk", [
            'post_id' => $post_id,
            'path' => $dest_file
        ]);
        return null;
    ...
    if (is_wp_error($attachment_id)) {
        stagekitwp_sync_log('error', "Failed to create season $image_type attachment", [
            'post_id' => $post_id,
    ...
    stagekitwp_sync_log('info', "Successfully attached season $image_type", [
        'post_id' => $post_id,
        'attachment_id' => $attachment_id,
        'filename' => $filename
    ]);
 ...
 */
function stagekitwp_sync_download_pdf($pdf_url, $filename) {
    if (empty($pdf_url) || empty($filename)) {
        stagekitwp_sync_log('warning', 'PDF download called with empty URL or filename');
    ...
    if (!function_exists('stagekitwp_sync_get_folder_id')) {
        require_once dirname(__FILE__) . '/sync/stagekitwp-sync-folder-discovery.php';
    }
    ...
    if (empty($pdf_url)) {
        stagekitwp_sync_log('warning', 'Could not extract URL from PDF field');
    ...
    stagekitwp_sync_log('debug', 'Downloading PDF from SharePoint using Graph API', [
    ...
    if (!class_exists('STAGEKITWP_Graph_Client')) {
        stagekitwp_sync_log('error', 'STAGEKITWP_Graph_Client not available for PDF download');
        error_log('[PDF_DEBUG] STAGEKITWP_Graph_Client not available');
        return false;
    }
    ...
    if (!$token) {
        stagekitwp_sync_log('error', 'Failed to get access token for PDF download');
    ...
    $folder_name = stagekitwp_sync_extract_folder_from_url($pdf_url);
    error_log('[PDF_DEBUG] Extracted folder name: ' . ($folder_name ? $folder_name : 'null'));
    
    if (!$folder_name) {
        stagekitwp_sync_log('warning', 'Could not extract folder name from PDF URL', ['url' => $pdf_url]);
    ...
    $folder_id = stagekitwp_sync_get_folder_id($folder_name, $site_id, $image_media_list_id, $token);
    error_log('[PDF_DEBUG] Folder ID result: ' . ($folder_id ? $folder_id : 'null'));
    
    if (!$folder_id) {
        stagekitwp_sync_log('error', 'Could not find PDF folder in Image Media', ['folder' => $folder_name]);
        error_log('[PDF_DEBUG] Could not find folder: ' . $folder_name);
        return false;
    }

    stagekitwp_sync_log('debug', 'Found PDF folder in Image Media', [
        'folder' => $folder_name,
        'folder_id' => $folder_id
    ]);
    ...
    if (is_wp_error($response)) {
        stagekitwp_sync_log('error', 'Failed to list PDF folder in Image Media', [
            'error' => $response->get_error_message(),
            'folder' => $folder_name
        ]);
    ...
    if (!isset($json['value'])) {
        stagekitwp_sync_log('error', 'Unexpected response from Graph API listing PDF folder', [
            'folder' => $folder_name
        ]);
    ...
    if (!$file_id) {
        stagekitwp_sync_log('warning', 'PDF file not found in SharePoint folder', [
    ...
    stagekitwp_sync_log('debug', 'Found PDF in Image Media library', [
        'filename' => $pdf_filename,
        'file_id' => $file_id
    ]);
    ...
    if (is_wp_error($response)) {
        stagekitwp_sync_log('error', 'Failed to download PDF from Image Media library', [
            'filename' => $filename,
            'error' => $response->get_error_message()
        ]);
    ...
    if ($http_code !== 200) {
        stagekitwp_sync_log('error', 'PDF download returned HTTP error', [
            'filename' => $filename,
            'http_code' => $http_code
        ]);
    ...
    if (empty($file_content)) {
        stagekitwp_sync_log('error', 'Downloaded PDF is empty', ['filename' => $filename]);
    ...
    if (strpos($file_content, '<!DOCTYPE') === 0 || strpos($file_content, '<html') === 0) {
        stagekitwp_sync_log('error', 'Downloaded content is HTML, not PDF', ['filename' => $filename]);
    ...
    if (strpos($file_content, '%PDF') !== 0) {
        stagekitwp_sync_log('warning', 'Downloaded file does not start with PDF marker', ['filename' => $filename]);
        error_log('[PDF_DEBUG] File does not start with %PDF marker');
        // Still save it anyway, might be valid
    }

    // Ensure folder exists
    $pdf_dir = stagekitwp_sync_get_images_dir();
    if (!file_exists($pdf_dir)) {
        if (!wp_mkdir_p($pdf_dir)) {
            stagekitwp_sync_log('error', 'Failed to create sync folder for PDF', ['path' => $pdf_dir]);
    ...
    if ($written === false) {
        stagekitwp_sync_log('error', 'Failed to write PDF file to disk', ['path' => $file_path]);
    ...
    stagekitwp_sync_log('info', 'Successfully downloaded and saved PDF', [
        'filename' => $filename,
 ...
 */
function stagekitwp_sync_get_pdf_url($filename) {
    if (empty($filename)) {
        return '';
    }
    ...
    $base_url = stagekitwp_sync_get_images_url();
 ...
 */
function stagekitwp_sync_generate_pdf_thumbnail($pdf_path, $attachment_id) {
    ...
    try {
        ...
        $thumb_url = stagekitwp_sync_get_images_url() . '/' . $thumb_filename;
        
        // Store thumbnail URL in attachment post meta
        if ($attachment_id > 0) {
            update_post_meta($attachment_id, '_stagekitwp_pdf_preview', $thumb_url);
        }
