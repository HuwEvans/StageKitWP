jQuery(document).ready(function($) {
    ...
    $('.stagekitwp-sync-button').on('click', function() {
        var $button = $(this);
        var cpt = $button.data('cpt');
        var $status = $('#stagekitwp-sync-status-' + cpt);
        var dryRun = $('#stagekitwp-sync-dry-run').is(':checked');

        $button.prop('disabled', true);
        updateStatus($status, 'Syncing...', 'info');

        $.ajax({
            url: stagekitwp_sync_vars.ajax_url,
            type: 'POST',
            data: {
                action: 'stagekitwp_sync_run',
                nonce: stagekitwp_sync_vars.nonce,
                cpt: cpt,
                dry_run: dryRun
            },
     ...
     */
    $('#stagekitwp-sync-ordered').on('click', function() {
        var $button = $(this);
        var $status = $('#stagekitwp-sync-ordered-status');
        var dryRun = $('#stagekitwp-sync-dry-run').is(':checked');

        $button.prop('disabled', true);
        $('.stagekitwp-sync-button').prop('disabled', true);
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'stagekitwp_sync_ordered',
                nonce: stagekitwp_sync_vars.nonce,
                dry_run: dryRun
            },
            success: function(response) {
            ...
            complete: function() {
                $button.prop('disabled', false);
                $('.stagekitwp-sync-button').prop('disabled', false);
            }
        });
    });

    $('#stagekitwp-sync-all').on('click', function() {
        var $button = $(this);
        var $globalStatus = $('#stagekitwp-sync-global-status');
        var dryRun = $('#stagekitwp-sync-dry-run').is(':checked');

        // Disable all buttons during sync
        $button.prop('disabled', true);
        $('.stagekitwp-sync-button').prop('disabled', true);
        $('#stagekitwp-sync-ordered').prop('disabled', true);
        
        updateStatus($globalStatus, 'Starting global sync...', 'info');

        // Get all CPTs from the existing buttons
        var cpts = $('.stagekitwp-sync-button').map(function() {
        ...
        function syncNext() {
            if (completedCpts >= cpts.length) {
                ...
                $button.prop('disabled', false);
                $('.stagekitwp-sync-button').prop('disabled', false);
                $('#stagekitwp-sync-ordered').prop('disabled', false);
                return;
            }

            var currentCpt = cpts[completedCpts];
            var $status = $('#stagekitwp-sync-status-' + currentCpt);
            updateStatus($status, 'Syncing...', 'info');
