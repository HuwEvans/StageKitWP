...
stagekitwp_sync_log('INFO','Generic-Image-Sync File loading');
 ...
 */
function stagekitwp_sync_find_attachment_by_filename($filename, $image_type = 'image') {
    global $wpdb;
    ...
    stagekitwp_sync_log('debug', "Searching for orphaned $image_type attachment", [
    ...
    if (empty($results)) {
        stagekitwp_sync_log('debug', "No attachments found with filename", [
            'search_filename' => $clean_filename
        ]);
        return null;
    }
    
    stagekitwp_sync_log('debug', "Found " . count($results) . " potential matches for $image_type", [
        'search_filename' => $clean_filename
    ]);
    ...
    if (empty($candidates)) {
        stagekitwp_sync_log('debug', "No matching attachments after filename comparison", [
            'search_filename' => $clean_filename,
    ...
    stagekitwp_sync_log('info', "Found existing $image_type attachment in media library", [
        'search_filename' => $clean_filename,
 ...
 * @param string $meta_key Meta key to store attachment ID (e.g., '_stagekitwp_logo', '_stagekitwp_photo')
 * @param string $image_type Type of image for logging (e.g., "photo", "logo")
 * @return int|null Attachment ID if found and exists, null otherwise
 */
function stagekitwp_sync_attachment_exists($post_id, $filename, $meta_key = '_stagekitwp_logo', $image_type = 'image') {
    ...
    if (!empty($existing_attachment_id) && is_numeric($existing_attachment_id)) {
        ...
        if ($attachment && $attachment->post_type === 'attachment') {
            ...
            if ($attached_file) {
                stagekitwp_sync_log('debug', "Found existing $image_type linked to post", array(
                    'filename' => $filename,
                    'meta_key' => $meta_key,
    ...
    $orphaned_attachment_id = stagekitwp_sync_find_attachment_by_filename($filename, $image_type);
    if ($orphaned_attachment_id) {
        stagekitwp_sync_log('info', "Found orphaned $image_type in media library (post may have been deleted)", array(
            'filename' => $filename,
 ...
 */
function stagekitwp_sync_download_image_from_media_library(
    $filename,
    $image_url,
    $filename_prefix,
    $site_id,
    ...
    stagekitwp_sync_log('debug', 'Downloading image from Image Media library', array(
        'filename' => $filename,
        'image_url' => $image_url
    ));

    // Discover folder ID from URL (with auto-caching)
    $folder_id = stagekitwp_sync_get_folder_id_from_image_url(
        $image_url,
        $site_id,
        $image_media_list_id,
        $token
    ...
    if (!$folder_id) {
        stagekitwp_sync_log('warning', 'Could not determine folder for image', [
            'filename' => $filename,
            'url' => $image_url
        ]);
        return null;
    ...
    stagekitwp_sync_log('debug', 'Using folder for image download', [
        'filename' => $filename,
        'folder_id' => $folder_id
    ]);
    ...
    if (is_wp_error($response)) {
        stagekitwp_sync_log('error', 'Failed to list folder in Image Media', [
    ...
    if (!$file_id) {
        stagekitwp_sync_log('warning', 'File not found in Image Media folder', [
            'filename' => $filename,
            'folder_id' => $folder_id
        ]);
        return null;
    ...
    stagekitwp_sync_log('debug', 'Found file in Image Media', [
        'filename' => $filename,
        'file_id' => $file_id
    ]);
    ...
    if (is_wp_error($response)) {
        stagekitwp_sync_log('error', 'Failed to download file from Image Media', [
            'filename' => $filename,
            'error' => $response->get_error_message()
        ]);
        return null;
    ...
    if (empty($file_content)) {
        stagekitwp_sync_log('error', 'Downloaded file is empty', [
            'filename' => $filename
        ]);
        return null;
    }

    stagekitwp_sync_log('debug', 'File downloaded from Image Media', [
        'filename' => $filename,
    ...
    if (!file_put_contents($dest_file, $file_content)) {
        stagekitwp_sync_log('error', 'Failed to write file to synced images folder', [
            'filename' => $filename,
            'dest' => $dest_file
        ]);
        return null;
    ...
    if (is_wp_error($attachment_id)) {
        stagekitwp_sync_log('error', 'Failed to insert attachment', [
            'filename' => $filename,
 ...
 * @return int|null Attachment ID if successful, null otherwise
 */
function stagekitwp_sync_image_for_post(
    $post_id,
    $filename,
    $image_url,
    $meta_key,
    ...
    stagekitwp_sync_log('debug', "Attempting to download $image_type from Image Media library", [
        'filename' => $filename,
        'post_id' => $post_id
    ]);

    // Check if we already have this attachment (checks post meta and global media library)
    $existing_attachment_id = stagekitwp_sync_attachment_exists($post_id, $filename, $meta_key, $image_type);
    if ($existing_attachment_id) {
        stagekitwp_sync_log('info', "Reusing existing $image_type attachment (not re-downloading)", array(
            'filename' => $filename,
            'attachment_id' => $existing_attachment_id,
            'post_id' => $post_id
        ));
    ...
    // Download the image
    $attachment_id = stagekitwp_sync_download_image_from_media_library(
        $filename,
        $image_url,
        "$image_type-$sp_id",
        $site_id,
    ...
    if ($attachment_id) {
        ...
        stagekitwp_sync_log('info', "Successfully attached $image_type", array(
            'filename' => $filename,
            'attachment_id' => $attachment_id,
            'post_id' => $post_id
        ));
        ...
        stagekitwp_sync_log('warning', "Could not sync $image_type", array(
            'filename' => $filename,
            'post_id' => $post_id,
